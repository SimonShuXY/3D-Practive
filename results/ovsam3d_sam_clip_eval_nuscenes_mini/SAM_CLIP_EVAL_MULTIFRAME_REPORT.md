...
- `multiframe_sam_open_vocab_bev.png`
- `multiframe_sam_open_vocab_points.ply`
...
In this five-sample diagnostic, SAM lowers the assigned-point ratio but improves
mapped accuracy on every sample. The next useful fix is label calibration rather
than more raw training: merge person/pedestrian variants, tune prompts for trailer
and construction vehicle, and report per-class precision / recall / IoU after the
mapping is stable.
